...
// SysCompany 公司表（参考 JeeSite js_sys_company，主键改为自增 + code 业务编码）
// ParentIds 为物化路径，格式如 "0,1,3,"，子树查询用前缀 LIKE
type SysCompany struct {
	global.GVA_MODEL
	Name      string       `json:"name" form:"name" gorm:"column:name;size:100;not null;comment:公司名称;" binding:"required"` //公司名称
	Code      string       `json:"code" form:"code" gorm:"column:code;size:64;not null;uniqueIndex;comment:公司编码;"`        //公司编码
	ParentId  uint         `json:"parentId" form:"parentId" gorm:"column:parent_id;index;default:0;comment:父公司ID 0为根;"`  //父公司ID
	...
	Children  []SysCompany `json:"children" gorm:"-"`                                                                    //子公司
}
