  ...
  <div>
    <GvaGrid ref="gridRef" v-bind="gridOptions" v-on="gridEvents">
      ...
      <template #operate="{ row }">
        <el-button type="primary" link icon="link" @click="openCurl(row)">Curl示例</el-button>
        <el-popover v-if="row.status" v-model:visible="row.visible" placement="top" width="160">
          <p>确定要作废吗？</p>
          <div style="text-align: right; margin: 0">
            <el-button size="small" type="primary" link @click="row.visible = false">取消</el-button>
            <el-button size="small" type="primary" @click="invalidateToken(row)">确定</el-button>
          </div>
          <template #reference>
            <el-button icon="delete" type="danger" link @click="row.visible = true">作废</el-button>
          </template>
        </el-popover>
      </template>
    </GvaGrid>
...
const { refresh } = useGvaGrid({
  id: 'systemTools-apiToken',
  api: getApiTokenList,
  defaultSort: null,
  searchItems: [
    { field: 'userId', title: '用户ID', span: 6, itemRender: { name: 'VxeInput', props: { placeholder: '搜索用户ID', clearable: true, type: 'number' } } },
    {
      field: 'status',
      title: '状态',
      span: 6,
  ...
  columns: [
    ...
    { field: 'user', title: '用户', minWidth: 150, slots: { default: 'user' } },
    { field: 'authorityId', title: '角色ID', width: 100 },
    { field: 'status', title: '状态', width: 100, slots: { default: 'status' } },
    { field: 'expiresAt', title: '过期时间', width: 180, slots: { default: 'expiresAt' } },
