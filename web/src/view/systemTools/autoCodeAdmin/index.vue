<template>
  <div>
    <GvaGrid ref="gridRef" v-bind="gridOptions" v-on="gridEvents">
      ...
      <template #flag="{ row }">
        <el-tag v-if="row.flag" type="danger" effect="dark">已回滚</el-tag>
        <el-tag v-else type="success" effect="dark">未回滚</el-tag>
      </template>

      <template #operate="{ row }">
        <div>
          <el-button type="primary" link :disabled="row.flag === 1" @click="addFuncBtn(row)">增加方法</el-button>
          <el-button type="primary" link @click="goAutoCode(row, 1)">增加字段</el-button>
          <el-button type="primary" link :disabled="row.flag === 1" @click="openDialog(row)">回滚</el-button>
          <el-button type="primary" link @click="goAutoCode(row)">复用</el-button>
...
<script setup>
  ...
  const { gridRef, gridOptions, gridEvents, refresh } = useGvaGrid({
    id: 'systemTools-autoCodeAdmin',
    api: getSysHistory,
    defaultSort: null,
    columns: [
      { field: 'ID', title: 'id', width: 60 },
      { field: 'CreatedAt', title: '日期', width: 180, cellRender: { name: 'gvaDate' } },
      { field: 'structName', title: '结构体名', minWidth: 150 },
      { field: 'description', title: '结构体描述', minWidth: 150 },
      { field: 'tableName', title: '表名称', minWidth: 150 },
      { field: 'flag', title: '回滚标记', minWidth: 150, slots: { default: 'flag' } },
      { title: '操作', fixed: 'right', slots: { default: 'operate' } }
    ]
  })

  const { deleteRow } = useGvaGridDelete(gridRef, {
    delete: (rows) => delSysHistory({ id: Number(rows[0].ID) }),
    confirmText: '此操作将删除本历史, 是否继续?',
    successText: '删除成功'
  })
