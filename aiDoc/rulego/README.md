...
> 本地参考克隆：`/home/changdq/src/gitee/ttshang/rulego`（fork，仅作对照参考，**不作为集成源**）
...
4. **主要代价**：gva 的 Go 版本需从 1.24 升到 **1.25**（rulego-server 的 go.mod 要求）；传递依赖树显著增大（components 六大生态模块 + eino/mcp-go 等）；`go.sum` 与二进制体积增长。
5. **前端**：**webui-vben 明确舍弃、不纳入集成范围**；规则链的可视化管理由 GVA 前端（Vue3 + Element Plus）自建页面承载。
6. **许可证无障碍**：双方均为 Apache 2.0，保留 LICENSE 与来源声明即可。

## 文档导航

| 文档 | 内容 |
| ---- | ---- |
| [01-集成方案分析.md](./01-集成方案分析.md) | 集成对象盘点、两侧架构对接分析、六种集成方式对比与选型、目录与 go.mod 规划、依赖与版本影响、治理纪律、许可证合规、风险清单 |
